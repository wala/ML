...
public class PandasReadExcelAnalysis {
  ...
  public static Map<InstanceKey, Set<String>> readExcelAnalysis(
      CallGraph CG, PointerAnalysis<? extends InstanceKey> PA, HeapModel H) {
    Set<CGNode> read_excel =
        CG.getNodes(
            MethodReference.findOrCreate(
                TypeReference.findOrCreate(
                    PythonTypes.pythonLoader, "Lpandas/functions/read_excel"),
          ...
          new PythonInstructionVisitor() {
            ...
            public void visitPythonInvoke(PythonInvokeInstruction inst) {
              if (!Collections.disjoint(
                  CG.getPossibleTargets(script, inst.getCallSite()), read_excel)) {
                PA.getPointsToSet(H.getPointerKeyForLocal(script, inst.getDef()))
                    .forEach(
                        (InstanceKey obj) -> {
      ...
      ir.visitAllInstructions(
          new PythonInstructionVisitor() {
            ...
            public void visitPropertyRead(AstPropertyRead instruction) {
              PA.getPointsToSet(H.getPointerKeyForLocal(script, instruction.getObjectRef()))
                  .forEach(
                      (InstanceKey obj) -> {
                        if (excelTableFields.containsKey(obj)) {
                          if (excelTableFields.containsKey(obj)) {
                            PA.getPointsToSet(
                                    H.getPointerKeyForLocal(script, instruction.getMemberRef()))
                                .forEach(
                                    (InstanceKey field) -> {
            ...
            public void visitGet(SSAGetInstruction instruction) {
              PA.getPointsToSet(H.getPointerKeyForLocal(script, instruction.getRef()))
                  .forEach(
                      (InstanceKey obj) -> {
                        if (excelTableFields.containsKey(obj)) {
                          excelTableFields
                              .get(obj)
                              .add(instruction.getDeclaredField().getName().toString());
                        }
                      });
            }
          });
