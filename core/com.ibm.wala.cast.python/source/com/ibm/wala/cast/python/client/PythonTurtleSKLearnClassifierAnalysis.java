...
public class PythonTurtleSKLearnClassifierAnalysis extends PythonTurtleAnalysisEngine {
  ...
  private static class ClassifierState {
    ...
    public String toString() {
      return "["
          + node.getMethod().getDeclaringClass().getName()
          + "("
          + node.getGraphNodeId()
          + "):"
          + vn
  ...
  public LabeledGraph<TurtlePath, EdgeType> performAnalysis(PropagationCallGraphBuilder builder)
    ...
    class ClassifierDomain extends MutableMapping<ClassifierState>
    ...
    class ClassifierFlowFunctions
      ...
      private boolean relevantFitCall(
        ...
        return flow.vn == vn && src.getNode() == flow.node && fitCalls.containsKey(pyCall);
      }
                      ...
                      (pb) -> {
                        ...
                        if (ret instanceof SSAReturnInstruction) {
                          if (ret.getNumberOfUses() > 0
                              && flow.vn == ret.getUse(0)
                              && flow.node == src.getNode()) {
                            ClassifierState adapt =
    ...
    class ClassifierProblem
        implements PartiallyBalancedTabulationProblem<
            BasicBlockInContext<IExplodedBasicBlock>, CGNode, ClassifierState> {
      ...
      private Collection<PathEdge<BasicBlockInContext<IExplodedBasicBlock>>> collectInitialSeeds() {
        ...
        for (BasicBlockInContext<IExplodedBasicBlock> bb : supergraph) {
          ...
          if (seeds.containsKey(instruction)) {
            ...
            ClassifierState fact =
                new ClassifierState(bb.getNode(), instruction.getDef(), State.FRESH);
      ...
      @Override
      public TabulationDomain<ClassifierState, BasicBlockInContext<IExplodedBasicBlock>>
          getDomain() {
        return domain;
      }
    ...
    PartiallyBalancedTabulationSolver<
        ...
        solver =
            PartiallyBalancedTabulationSolver.createPartiallyBalancedTabulationSolver(
                new ClassifierProblem(), null);
    TabulationResult<BasicBlockInContext<IExplodedBasicBlock>, CGNode, ClassifierState> result =
        solver.solve();

    Graph<BasicBlockInContext<IExplodedBasicBlock>> sg =
        GraphInverter.invert(result.getProblem().getSupergraph());

    IFlowFunctionMap<BasicBlockInContext<IExplodedBasicBlock>> functionMap =
        result.getProblem().getFunctionMap();
    ...
    class Backtrace {
      private boolean hasCallee(BasicBlockInContext<IExplodedBasicBlock> returnSite) {
        ...
        for (Iterator<? extends BasicBlockInContext<IExplodedBasicBlock>> it =
                supergraph.getPredNodes(returnSite);
            it.hasNext(); ) {
      ...
      private IntSet check(
          IUnaryFlowFunction f, BasicBlockInContext<IExplodedBasicBlock> pred, int d2) {
                ...
                (p) -> {
                  switch (supergraph.classifyEdge(p, f.fst)) {
                    case ISupergraph.CALL_EDGE:
                      {
                        ...
                        check(ff, p, domain.getMappedIndex(f.snd))
                            .foreach(
                                (d1) -> {
                    ...
                    case ISupergraph.RETURN_EDGE:
                      {
                        Iterator<? extends BasicBlockInContext<IExplodedBasicBlock>> calls =
                            supergraph.getCallSites(f.fst, p.getNode());
                        while (calls.hasNext()) {
                          IUnaryFlowFunction ff =
                              (IUnaryFlowFunction)
                                  functionMap.getReturnFlowFunction(calls.next(), p, f.fst);
                          check(ff, p, domain.getMappedIndex(f.snd))
                              .foreach(
                                  (d1) -> {
                                    result.add(new FactPair(p, domain.getMappedObject(d1)));
      ...
      Iterable<FactPair> trace(
          BasicBlockInContext<IExplodedBasicBlock> node, ClassifierState state) {
                          ...
                          if (s.vn == vn && s.node == bbic.getNode()) {
                            LOGGER.info(
                                ((AstMethod) bbic.getMethod())
                                    ...
                                    + " : "
                                    + (s.state.equals(State.FIT) ? "is fitted" : "not fitted"));
                            SortedSet<Position> trace = new TreeSet<Position>();
                            bt.trace(bbic, s)
                                .forEach(
                                    (fp) -> {
                                      if (fp.fst.getMethod() instanceof AstMethod
                                          && fp.fst.getLastInstruction() != null) {
                                        trace.add(
                                            ((AstMethod) fp.fst.getMethod())
                                                .debugInfo()
